...
class Other_RoteMemory:
    ...
    def create(self):
        query = f"select ROW_NUMBER() OVER(ORDER BY term Asc) AS Row, term , value from german where (sense not like   '%ubstantiv%'  and sense not like '%Verb%') and ktype = 'reverso_senses';"
        ...
        for row in records:
            ...
            if len(english) > 0: # there may be reverso, but no duden !!
                ...
                query_B = f"select value from german where (sense not like   '%ubstantiv%'  and sense not like '%Verb%') and ktype = 'duden' and term = '{term}';"
